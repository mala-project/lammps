...
do i = 1,Nr_atoms
  if (Element_Type(i).eq.'H') then
    ...
    if (Element_Type(i).eq.'U') then
      Znuc(i) = 92.D0  ! For Uranium
      Mnuc(i) = 238.0D0
    endif
  endif
enddo
...
! get RX, RY, YZ form lammps

subroutine Initiate_lmp(Coulomb_acc,TIMERATIO,LBox,RX,RY,RZ,Nr_atoms, &
  H_INDEX_START,H_INDEX_END,COULCUT,Element_Type,Znuc,Mnuc,NrOrb)
...
CNT = 1
do i = 1,Nr_atoms
  !write(6,*) 'element type=', Element_type(i)
