...
SUBROUTINE LATTEQEQ(NTYPES, TYPES, CR_IN, MASSES_IN, XLO, XHI, XY, XZ, YZ, FORCES, &
       MAXITER_IN, VENERG, VEL_IN, DT_IN, VIRIAL_INOUT, CURRENTSTEP, LOCAL_CHI_IN, &
...
REAL(PREC), INTENT(IN):: LOCAL_CHI_IN(:) !NOTE james changed from:LOCAL_CHI_IN(:,:) 5/16/23
...
integer,    parameter :: LL = 8  !! LL = max number of Rank-m
integer,    parameter :: m = 8, MaxIt = 20 
real(PREC), parameter :: T = 300.D0, C_mix = 0.1D0, FelTol = 0.001D0  ! FelTol is convergence required for Relative Err in KK0Res
...
real(PREC)            :: Err_qn, Err_qxn, Err_ECoul, TR_CC !! ANDERS NEW FEB 9 2023
...
if (.not.allocated(ATELE)) THEN
  ...
  ALLOCATE(RX(NATS), RY(NATS), RZ(NATS), q(NATS), qx(NATS), qqx(NATS), qqqx(NATS)) 
...
!if (.true.) then
  ...
  print *, LOCAL_CHI_IN(k1), LOCAL_CHI_IN(k2), LOCAL_CHI_IN(k3), LOCAL_CHI_IN(k4), &
          LOCAL_CHI_IN(k5), LOCAL_CHI_IN(k6), LOCAL_CHI_IN(k7), LOCAL_CHI_IN(k8), &
          LOCAL_CHI_IN(k9), LOCAL_CHI_IN(k10), LOCAL_CHI_IN(k11), LOCAL_CHI_IN(k12), &
          LOCAL_CHI_IN(k13), LOCAL_CHI_IN(k14), LOCAL_CHI_IN(k15), LOCAL_CHI_IN(k16), &
          LOCAL_CHI_IN(k17), LOCAL_CHI_IN(k18), LOCAL_CHI_IN(k19), LOCAL_CHI_IN(k20), &
          LOCAL_CHI_IN(k21), LOCAL_CHI_IN(k22), LOCAL_CHI_IN(k23), LOCAL_CHI_IN(k24)
  bb = 0.d0
  DO I = 1, NATS
    ! use lammps value
    bb(I) = - LOCAL_CHI_IN(I) !NOTE James changed from: LOCAL_CHI_IN(I,1) on 5/16/24
    !bb(I) =  LOCAL_CHI_IN(I) !NOTE James tested: LOCAL_CHI_IN(I) on 5/20/25 
    ! use constant
...
do MD_step = 1,MD_Iter
  ...
  write(6,*) 'currentstep kk0freq', currentstep,KK0FREQ,mod(currentstep,KK0FREQ)
  if (mod(currentstep,KK0FREQ).EQ.0 .or. currentstep .eq. 1 .or. currentstep .eq. 0) then    !! CHANGE ANDERS
     ReCalc = 1                      !! CHANGE ANDERS
  endif                              !! CHANGE ANDERS
  if (mod(currentstep,KK0FREQ).EQ.0 .or. currentstep .eq. 1 .or. currentstep .eq. 0) then  !! CHANGE ANDERS
  !if (mod(currentstep,2).EQ.0) then    !! CHANGE ANDERS
  !   ReCalc = 1                      !! CHANGE ANDERS
  !endif                              !! CHANGE ANDERS
  !if (mod(currentstep,2).EQ.0) then  !! CHANGE ANDERS
  !if (ReCalc.EQ.1) then                 !! CHANGE ANDERS
     call nearestneighborlist(nrnnlist,nndist,nnRx,nnRy,nnRz,nnType,nnStruct,nrnnStruct,RX,RY,RZ,LBox,COULCUT, &
                            NATS,Max_Nr_Neigh)
     !write(6,*) 'CoulombMatrix calculation'
     call CoulombMatrix(CC,RX,RY,RZ,LBox,Hubbard_U,ATELE,NATS,Coulomb_acc,TIMERATIO, &
...
!              ' ERR ECOUL = ', Err_ECoul/(One*MD_step), Hubbard_U(1), Hubbard_U(2), Hubbard_U(3)
!   write(*,*) ' TR_CC = ', TR_CC
   !write(*,*) ' Coulomb_Pot = ', Coulomb_Pot(1:3)
   !write(*,*) ' Coulomb_Pot_Exact = ', Coulomb_Pot_Exact(1:3)
...
!! KRes = 0 first time CHANGE ANDERS
  dn2dt2 = KRes
!  if (MD_step>100) then
!     alpha = 0.D0
!  endif
...
!!$OMP END PARALLEL DO
  ...
  if (printcharges) then
     if (exact_solution) then
             ! q[n], 
        write(24,'(f9.4,10000f15.10)')  Time/1000, qx, q, n
     else
        write(24,'(f9.4,10000f15.10)')  Time/1000, qx 
     endif
  endif
...
!
  ...
  do I = 1,NATS
    call Ewald_Real_Space(Coulomb_Pot_Real_I,Coulomb_Force_Real_I,I,RX,RY,RZ,LBox, &
    qqx,Hubbard_U,ATELE,NATS,Coulomb_acc,TIMERATIO,nnRx,nnRy,nnRz,nrnnlist,nnType,HDIM,Max_Nr_Neigh)
    Coulomb_Pot_Real(I) = Coulomb_Pot_Real_I
    Coulomb_Force_Real(:,I) = Coulomb_Force_Real_I(:)
  enddo
  call Ewald_k_Space(Coulomb_Pot_k,Coulomb_Force_k,RX,RY,RZ,LBox,n,NATS,Coulomb_acc,TIMERATIO,Max_Nr_Neigh)
  ...
  !write(6,*) 'lambda =', lambda
  do I = 1,NATS
     qqqx(I) = (bb(I)-Coulomb_Pot(I) + lambda)/Hubbard_U(I)
  enddo
!!!!!!!!!!!!!! CHANGE END ANDERS
  ...
  do I = 1,NATS
    ECoul = ECoul  - qqqx(I)*bb(I) + 0.5D0*qqqx(I)*Hubbard_U(I)*qqqx(I) + 0.5D0*(2.D0*qqqx(I)-qqx(I))* Coulomb_Pot(I)  !! CHANGE ANDERS
  ...
  do I = 1, NATS
    ...
    Coulomb_Force(1,I) = (2.D0*qqqx(I)-qqx(I))*Coulomb_Force(1,I)/qqx(I)  ! CHANGE ANDERS
    Coulomb_Force(2,I) = (2.D0*qqqx(I)-qqx(I))*Coulomb_Force(2,I)/qqx(I)  ! CHANGE ANDERS
    Coulomb_Force(3,I) = (2.D0*qqqx(I)-qqx(I))*Coulomb_Force(3,I)/qqx(I)  ! CHANGE ANDERS
  enddo
  ...
  VENERG = EPOT
   ...
   write(*,*) ' Err |q-n| = ', NORM2(q-n)/(SQRT(ONE*NATS)), ' Err |q[n]-n| ', NORM2(qx-n)/(SQRT(ONE*NATS)), &
              ' ERR ECOUL = ', ABS(ECoul_Exact-ECoul)
